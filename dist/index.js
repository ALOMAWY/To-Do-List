...
let tasksNumberSpan = document.getElementById("tasks-number");
...
function createNewTask(title, id) {
    ...
    taskDate.innerText = `${date.getDate()} / ${date.getMonth() + 1} / ${date.getFullYear()} `;
  ...
  <i class="fa-solid fa-trash btn btn-danger"></i>`;
    ...
    list === null || list === void 0 ? void 0 : list.prepend(task);
}
...
function createRealTaskOnPage(tasks) {
    ...
    if (tasksNumberSpan)
        tasksNumberSpan.innerHTML = `${tasks.length}`;
}
